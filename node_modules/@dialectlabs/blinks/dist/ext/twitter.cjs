...
var _chunkNGCIS6IQcjs = require('../chunk-NGCIS6IQ.cjs');
...
async function handleNewNode(node, config, callbacks, options) {
  ...
  if (interstitialData.isInterstitial) {
    ...
    const actionsJson = await fetch(_blinkscore.proxify.call(void 0, actionsJsonUrl)).then(
      (res) => res.json()
    );
